...
2026-09-13：當時的13份現行範例均補入實體接軌側別 `fromPortSide`／`toPortSide`。七種站型移除未宣告的多餘轉向；站後折返修正渡線端點與實際去回路徑。兩份V3.3及baseline的尾軌／袋狀軌新增各180m進出軌，因此運行時間可能較舊範例增加；baseline中央上行月台及西端連接一併修正。最新主圖／編輯器均無配線警告。2026-09-20 新增臺中機場捷運 full scenario 後，現行總數為14份。
...
| `臺中機場捷運-完整營運示範範例.mrtsim.json` | 臺中機場捷運 `full` operational scenario | O01～O26（含 O08a／O15a）完整邏輯站鏈、FULL-LINE／SECTION／AIRPORT-DIRECT、O20 站後 pocket 概念與 O04／O13 越行情境；照片彙整只作規劃情境依據，其餘模型值均須以 Manifest 的 synthetic 分界為準。 |
...
自動化 sample gate 目前涵蓋 14 個現行範例（含下列七個 PDF 站型）；一般範例以 topology-native `SimulationWorld` 推進 3,600 秒，檢查未使用 edge、legacy facility edge 欄位、未具名續行班次、碰撞、停站違規及列車退出。臺中機場捷運 full scenario 建議使用 8,000 秒有界模擬時間，以涵蓋 O04／O13 越行、O20 折返與後續退出。這些 gate 與情境測試不代表正式路線、工程配線或安全設計。
...
### 臺中機場捷運完整營運示範範例 Manifest

- 案例與層級：`臺中機場捷運-完整營運示範範例`，`full`；這是可重建、可載入的 Schema 8 topology sample，不是正式工程模型。
- 資料來源與品質：`外部檔案參考/Taichung_Airport_MRT_simulation_reference.md` 彙整使用者提供的規劃服務構想頁面照片。它可支持規劃情境與概念性營運需求，但不是正式報告 PDF、平縱面、配線圖、車輛規格或號誌設計文件；任何工程化解讀都必須另以正式來源覆核。
- 車站與路線情境：邏輯站鏈為 O01～O26，另含 O08a、O15a；本案例採 O08a 位於 O08 後、O15a 位於 O15 後的 builder 順序。O01／O16 的臺中機場／臺中車站名稱及 O16 地下疊式月台、轉乘關係屬 source-backed planning metadata；其餘站名、逐站里程與精確位置不視為正式資料。
- source-backed planning scenario：
  - 全程路線約 29.9 km；O01～O20 約 23.8 km；O08～O20 約 13.9 km，僅作 aggregate reasonableness targets。
  ...
  - O04／O13 是 2 側式月台＋4 股道的雙向越行／待避概念；O04 為高架、O13 為地下。1.5 分鐘是營運配置目標，不取代 moving block、occupancy、braking 或 rear-clear 安全模型。
- synthetic test values／非正式設計值：
  - 逐站距離分配、每條 edge 長度、精確 chainage、O04／O13 道岔位置與 crossover 幾何。
...
- 刻意省略或尚未可工程化：正式逐站里程與平縱面、曲線／坡度／曲線限速、正式 turnout 型號與岔速、正式平台有效長度、正式車輛／號誌／閉塞參數、正式尖峰／離峰 timetable、fleet cycle／layover、完整容量客流模型及 O16 樓層／轉乘的列車 physics。O04／O13 的四股道是 source-backed planning concept；目前可執行 facility 的細部幾何、port side 與方向對稱性仍屬 synthetic，不能視為工程配線。
...
- 目前驗證狀態（2026-09-20）：builder 的 7 個階段 Structural／Operational gate、focused Taichung scenario tests 15/15、14-sample gate、Release build（0 warnings／0 errors）、完整 Engine runner 160/160 及 WPF runner 均通過；WPF 含全 14 份 sample 的載入／計畫時間軸／雙向預覽，以及本 sample 主圖／編輯器 720／1200px 配置檢核。原生桌面不同 DPI、O04／O13 越行與 O20 折返的連續播放目視仍未執行，不能用離屏 runner 代替。
- 已知限制：此案例的通過結果只代表 synthetic topology-native sample 可執行，以及目前列出的 runtime 事件可被驗證；不代表臺中機場捷運正式設計、實際站距、四股道配線、號誌安全能力、正式容量或營運時刻表。
